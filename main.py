...
def read_root():
        ...
        # JD insights
        llm = LLMService()
        jd_insights = llm.jd_insights(jd)
        jd_insights = jd_insights['message']['content']
        ...
        experience = llm.resume_work_experience(docs[0])
        experience = experience['message']['content']

        # Other meta data of a resume
        insights = llm.resume_insights(docs[0])
        insights = insights['message']['content']

        print(insights)
        insights = json.loads(insights)
        insights["experience"]=experience

        # Scoring the resume
        res = llm.scoring_query(jd_insights, insights)    
